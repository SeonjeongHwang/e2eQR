...
def get_musique_features(data_file, tokenizer, mode):
    def find_order(tree, start, num_fact):
        connections = dict()
        for (a, b) in tree:
        ...
        order = [start]
        remain = set(list(connections.keys())) - set([start])
        while len(order) < num_fact:
            avails = []
            for prev in order:
                for candi in connections[prev]:
                    if candi not in order:
                        avails.append(candi)
            order.append(avails[0])
            remain = remain - set([avails[0]])
    ...
    for data in tqdm.tqdm(data_list):
        ...
        question = data["question"]
        answer = data["answer"]
        answer_fact_idx = data["question_decomposition"][-1]["paragraph_support_idx"]

        if data["tree"] is not None and not args.no_arrangement:
            order = find_order(data["tree"], answer_fact_idx, hop)
            id_to_fact_order[data["id"]] = order

            input_seq_list = []
            ### initial
            bridges = [ent["text"] for ent in data["bridges"][str(answer_fact_idx)]]
            input_seq = "Answer: " + answer + " Fact: " + pid_to_fact[answer_fact_idx] + " Bridges: " + " ".join(list(bridges))
            input_seq_list.append(input_seq)

            used_facts = [answer_fact_idx]
            for f_idx in order[1:]:
                bridges = [ent["text"] for ent in data["bridges"][str(f_idx)] if ent["neighbor_fact"] not in used_facts]
                used_facts.append(f_idx)

                if len(bridges):
                    input_seq = "Fact: " + pid_to_fact[f_idx] + " Bridges: " + " ".join(list(bridges))
                else:
                    input_seq = "Fact: " + pid_to_fact[f_idx]
                input_seq_list.append(input_seq)
            ...
            while len(used_facts) < len(pid_to_fact.keys()):
                ...
                if args.no_cross_attention:
                    input_seq = "Answer: " + answer + " " + input_seq
    ...
    with open(f"{args.checkpoint}-order.json", "w") as fout:
        json.dump(id_to_fact_order, fout, indent=1)
...
class GDataset(Dataset):
    def __init__(self, data_file, tokenizer, device, mode):
        ...
        if "hotpot" in data_file.lower():
            self.features_per_hop, self.unique_to_id, self.unique_to_gold = get_hotpot_features(data_file, tokenizer, mode=mode)
        else:
...
class Model(nn.Module):
    ...
    def forward(self, hop, total_input_ids, total_attention_mask, decoder_input_ids, decoder_attention_mask, labels):
        ...
        for i in range(hop-1):
            ...
            intermediate_decoder_attention_mask = torch.cat(accum_decoder_attention_mask, dim=-1) if accum_decoder_attention_mask else None
            ...
            if args.no_self_attention:
                ...
                if past_key_values is not None:
                    past_key_values = tuple([tuple([None, None, kv[2], kv[3]]) for kv in past_key_values])
            if args.no_cross_attention:
                cross_attention_mask = attention_mask_per_hop[i]
                if past_key_values is not None:
                    past_key_values = tuple([tuple([kv[0], kv[1], None, None]) for kv in past_key_values])
            OutputDict = self.EncoderDecoder.intermediate_forward(input_ids=input_ids_per_hop[i],
                                                                  ...
                                                                  decoder_attention_mask=intermediate_decoder_attention_mask,
...
#
        ...
        intermediate_decoder_attention_mask = torch.cat(accum_decoder_attention_mask, dim=-1)
        cross_attention_mask = torch.cat(accum_cross_attention_mask, dim=-1)
        if args.no_self_attention:
            intermediate_decoder_attention_mask = decoder_attention_mask
            if past_key_values is not None:
                past_key_values = tuple([tuple([None, None, kv[2], kv[3]]) for kv in past_key_values])
        if args.no_cross_attention:
            cross_attention_mask = attention_mask_per_hop[-1]
            if past_key_values is not None:
                past_key_values = tuple([tuple([kv[0], kv[1], None, None]) for kv in past_key_values])
    ...
    def generate(self, hop, total_input_ids, total_attention_mask):
        ...
        intermediate_decoder_attention_mask = torch.cat(accum_decoder_attention_mask, dim=-1) if len(accum_decoder_attention_mask) > 0 else None
        cross_attention_mask = torch.cat(accum_cross_attention_mask, dim=-1)
        if args.no_self_attention:
            intermediate_decoder_attention_mask = None
            if past_key_values is not None:
...
def get_scores(predictions, references):
    ...
    for pred, gold in zip(predictions, references):
        if pred.strip() == "" or len(pred.strip()) == 1:
            pred = "empty"
