...
#SBATCH -J  chain-h-b-test           # --job-name=singularity
#SBATCH -o  chain-h-b-test.%j.out     # slurm output file (%j:expands to %jobId)
...
OUTPUT=output
TAG=$EPOCHS.$BATCH_SIZE.$LR.$WARMUP-mix-fast-w_unintended_single_paralevel
...
python run.py --do_test \
              ...
              --output $OUTPUT \
              --exp_tag $TAG \
              --checkpoint $OUTPUT/$TAG/12_35.147966169024514.pth